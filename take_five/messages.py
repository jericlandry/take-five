...
PREP_PACKET_SYSTEM = """You are T5, the Take Five care coordination assistant.
You are generating a pre-visit appointment prep packet for a family member 
who is accompanying a senior to a medical appointment.

Your output must be a clean, scannable checklist — not a narrative document.
The person reading this will be on their phone in a waiting room.
Be specific and concise. Always attribute observations to the person who made them and when.
Filter all items through the doctor's specialty — only include concerns relevant to this provider.
Do not include clinical jargon. Write like a smart, caring family member, not a medical record."""
...
async def generate_prep_packet(
    ...
    - followup_text is the short follow-up prompt asking for additions and who's accompanying
    """
    today = datetime.now().strftime("%B %d, %Y")
    ...
    # -- Step 2: Look up doctor specialty from clinical records --
    specialty = None
    specialty_line = ""
    specialty_short = ""
    try:
        care_team = repo.get_clinical_records_for_circle(
            circle_id, resource_type="CareTeamMember"
        )
        for rec in care_team:
            data = rec["data"] if isinstance(rec["data"], dict) else json.loads(rec["data"])
            name = data.get("name", "")
            if doctor_name.lower().replace("dr.", "").strip() in name.lower():
                specialty = data.get("specialty")
                break
        if specialty:
            specialty_line = f" ({specialty})"
            specialty_short = f" · {specialty}"
            logger.info(f"[prep_packet] Specialty found: {specialty}")
        else:
            logger.info(f"[prep_packet] No specialty found for {doctor_name}")
    except Exception as e:
        logger.warning(f"[prep_packet] Could not look up specialty: {e}")
    ...
    for msg in reversed(all_recent):  # oldest first
        body = (msg.get("body") or "").lower()
        if doctor_search in body and any(kw in body for kw in ["appointment", "visit", "saw", "appt"]):
            msg_date = msg.get("sent_at")
            if msg_date and isinstance(msg_date, datetime):
                # Make naive datetimes timezone-aware
                if msg_date.tzinfo is None:
                    msg_date = msg_date.replace(tzinfo=timezone.utc)
                if msg_date < datetime.now(timezone.utc) - timedelta(days=1):
                    prior_visit_date = msg_date
                    break

    if prior_visit_date:
        lookback_start = prior_visit_date
        lookback_desc = f"since last visit with {doctor_name} on {prior_visit_date.strftime('%B %d, %Y')}"
        logger.info(f"[prep_packet] Prior visit found: {prior_visit_date}")
    else:
        lookback_start = datetime.now(timezone.utc) - timedelta(days=60)
        lookback_desc = f"last 60 days (no prior visit with {doctor_name} found in chat history)"
    ...
    human_content = PREP_PACKET_PROMPT.format(
        ...
        doctor_name_upper=doctor_name.upper(),
        specialty_line=specialty_line,
        specialty_short=specialty_short,
        appointment_desc=appointment_desc,
        lookback_desc=lookback_desc,
        roster=ctx_builder._roster,
        clinical_records=ctx_builder._clinical,
    ...
    # This allows retrieval later via /app/circles/{id}/prep-packets
    try:
        ...
        if circle_ext_id:
            ...
            logger.info(f'[prep_packet] Logged to messages table')
    except Exception as e:
    ...
    return packet_text, followup_text
